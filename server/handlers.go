...
func upload_ragequit(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	case "GET":
		...
		for rows.Next() {
			...
			ctr++
			fmt.Fprintf(w, "#%d: %d:%d:%d [%d|%d]\n", ctr, (ragequit.Time / 3600), (ragequit.Time/60)%60, ragequit.Time%60, ragequit.PositionX, ragequit.PositionY)
		}
...
func upload_win(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	case "GET":
		...
		for rows.Next() {
			...
			ctr++
			fmt.Fprintf(w, "#%d: %d:%d:%d\n", ctr, (win.Time / 3600), (win.Time/60)%60, win.Time%60)
		}
...
func upload_death(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	case "GET":
		...
		for rows.Next() {
			...
			ctr++
			fmt.Fprintf(w, "#%d: %d:%d:%d [%d|%d]\n", ctr, (death.Time / 3600), (death.Time/60)%60, death.Time%60, death.PositionX, death.PositionY)
		}
